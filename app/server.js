...
init().then(() => {
    ...
    let ssoConfig = {
        clientID: config.secret_names.okta.clientId,
        clientSecret: config.secret_names.okta.clientSecret,
        callbackURL: config.secret_names.okta.redirect,
        scope: ['openid', 'profile', 'email'],
        authorizationURL: config.secret_names.okta.authorizationURL,
        tokenURL: config.secret_names.okta.tokenURL,
        issuer: config.secret_names.okta.issuer,
        userInfoURL: config.secret_names.okta.userInfoURL,
    };
    const ssoService = new SSOService(ssoConfig);
    server.use(ssoService.initialize());
    server.use(ssoService.session());
    new AuthRoute(server, ssoService);
});
